...
import (
	...
	"github.com/kubesphere/kubekey/controlplane/kubeadm/internal/etcd"
	etcdutil "github.com/kubesphere/kubekey/controlplane/kubeadm/internal/etcd/util"
...
func (w *Workload) updateManagedEtcdConditions(ctx context.Context, controlPlane *ControlPlane) {
	...
	for _, node := range controlPlaneNodes.Items {
		...
		if !machine.ObjectMeta.DeletionTimestamp.IsZero() {
			conditions.MarkFalse(machine, controlplanev1.MachineEtcdMemberHealthyCondition, kubekeyv1.DeletingReason, kubekeyv1.ConditionSeverityInfo, "")
			continue
		}
		...
		if !etcdutil.MemberEqual(members, currentMembers) {
			conditions.MarkFalse(machine, controlplanev1.MachineEtcdMemberHealthyCondition, controlplanev1.EtcdMemberUnhealthyReason, kubekeyv1.ConditionSeverityError, "etcd member reports the cluster is composed by members %s, but all previously seen etcd members are reporting %s", etcdutil.MemberNames(currentMembers), etcdutil.MemberNames(members))
			continue
		}
		...
		if len(member.Alarms) > 0 {
			...
			if len(alarmList) > 0 {
				conditions.MarkFalse(machine, controlplanev1.MachineEtcdMemberHealthyCondition, controlplanev1.EtcdMemberUnhealthyReason, kubekeyv1.ConditionSeverityError, "Etcd member reports alarms: %s", strings.Join(alarmList, ", "))
				continue
			}
		}
		...
		if *clusterID != member.ClusterID {
			conditions.MarkFalse(machine, controlplanev1.MachineEtcdMemberHealthyCondition, controlplanev1.EtcdMemberUnhealthyReason, kubekeyv1.ConditionSeverityError, "etcd member has cluster ID %d, but all previously seen etcd members have cluster ID %d", member.ClusterID, *clusterID)
			continue
		}
	...
	aggregateFromMachinesToKCP(aggregateFromMachinesToKCPInput{
		...
		machineConditions: []kubekeyv1.ConditionType{controlplanev1.MachineEtcdMemberHealthyCondition},
		kcpErrors:         kcpErrors,
...
func (w *Workload) getCurrentEtcdMembers(ctx context.Context, machine *kubekeyv1.Machine, nodeName string) ([]*etcd.Member, error) {
	...
	if len(etcdClient.Errors) > 0 {
		conditions.MarkFalse(machine, controlplanev1.MachineEtcdMemberHealthyCondition, controlplanev1.EtcdMemberUnhealthyReason, kubekeyv1.ConditionSeverityError, "Etcd member status reports errors: %s", strings.Join(etcdClient.Errors, ", "))
	...
	if err != nil {
		...
		conditions.MarkFalse(machine, controlplanev1.MachineEtcdMemberHealthyCondition, controlplanev1.EtcdMemberUnhealthyReason, kubekeyv1.ConditionSeverityError, "Failed get answer from the etcd member on the %s node", nodeName)
...
func compareMachinesAndMembers(controlPlane *ControlPlane, members []*etcd.Member, kcpErrors []string) []string {
	...
	for _, machine := range controlPlane.Machines {
		...
		if !found {
			conditions.MarkFalse(machine, controlplanev1.MachineEtcdMemberHealthyCondition, controlplanev1.EtcdMemberUnhealthyReason, kubekeyv1.ConditionSeverityError, "Missing etcd member")
		}
	}
...
func (w *Workload) UpdateStaticPodConditions(ctx context.Context, controlPlane *ControlPlane) {
	allMachinePodConditions := []kubekeyv1.ConditionType{
	...
	for _, node := range controlPlaneNodes.Items {
		...
		if !machine.ObjectMeta.DeletionTimestamp.IsZero() {
			for _, condition := range allMachinePodConditions {
				conditions.MarkFalse(machine, condition, kubekeyv1.DeletingReason, kubekeyv1.ConditionSeverityInfo, "")
			}
			continue
		}
	...
	for i := range controlPlane.Machines {
		...
		if !found {
			for _, condition := range allMachinePodConditions {
				conditions.MarkFalse(machine, condition, controlplanev1.PodFailedReason, kubekeyv1.ConditionSeverityError, "Missing node")
			}
		}
	}
...
func (w *Workload) updateStaticPodCondition(ctx context.Context, machine *kubekeyv1.Machine, node corev1.Node, component string, staticPodCondition kubekeyv1.ConditionType) {
	...
	if err := w.Client.Get(ctx, podKey, &pod); err != nil {
		...
		if apierrors.IsNotFound(err) {
			conditions.MarkFalse(machine, staticPodCondition, controlplanev1.PodMissingReason, kubekeyv1.ConditionSeverityError, "Pod %s is missing", podKey.Name)
			return
		}
	...
	case corev1.PodPending:
		...
		if podCondition(pod, corev1.PodScheduled) != corev1.ConditionTrue {
			conditions.MarkFalse(machine, staticPodCondition, controlplanev1.PodProvisioningReason, kubekeyv1.ConditionSeverityInfo, "Waiting to be scheduled")
			return
		}
		...
		if podCondition(pod, corev1.PodInitialized) != corev1.ConditionTrue {
			conditions.MarkFalse(machine, staticPodCondition, controlplanev1.PodProvisioningReason, kubekeyv1.ConditionSeverityInfo, "Running init containers")
			return
		}

		// If there are no error from containers, report provisioning without further details.
		conditions.MarkFalse(machine, staticPodCondition, controlplanev1.PodProvisioningReason, kubekeyv1.ConditionSeverityInfo, "")
	case corev1.PodRunning:
		...
		if len(containerWaitingMessages) > 0 {
			if terminatedWithError {
				conditions.MarkFalse(machine, staticPodCondition, controlplanev1.PodFailedReason, kubekeyv1.ConditionSeverityError, strings.Join(containerWaitingMessages, ", "))
				return
			}
			...
			conditions.MarkFalse(machine, staticPodCondition, controlplanev1.PodProvisioningReason, kubekeyv1.ConditionSeverityInfo, strings.Join(containerWaitingMessages, ", "))
			return
		}
		...
		if len(containerTerminatedMessages) > 0 {
			conditions.MarkFalse(machine, staticPodCondition, controlplanev1.PodFailedReason, kubekeyv1.ConditionSeverityError, strings.Join(containerTerminatedMessages, ", "))
			return
		}
		...
		conditions.MarkFalse(machine, staticPodCondition, controlplanev1.PodProvisioningReason, kubekeyv1.ConditionSeverityInfo, "Waiting for startup or readiness probes")
	case corev1.PodSucceeded:
		// PodSucceeded means that all containers in the pod have voluntarily terminated
		// with a container exit code of 0, and the system is not going to restart any of these containers.
		// NOTE: This should never happen for the static pods running control plane components.
		conditions.MarkFalse(machine, staticPodCondition, controlplanev1.PodFailedReason, kubekeyv1.ConditionSeverityError, "All the containers have been terminated")
...
type aggregateFromMachinesToKCPInput struct {
	controlPlane      *ControlPlane
	machineConditions []kubekeyv1.ConditionType
	kcpErrors         []string
	condition         kubekeyv1.ConditionType
...
func aggregateFromMachinesToKCP(input aggregateFromMachinesToKCPInput) {
	...
	if len(input.kcpErrors) > 0 {
		conditions.MarkFalse(input.controlPlane.KCP, input.condition, input.unhealthyReason, kubekeyv1.ConditionSeverityError, strings.Join(input.kcpErrors, "; "))
		return
	}

	// In case of no errors and at least one machine with warnings, report false, warnings.
	if len(kcpMachinesWithWarnings) > 0 {
		conditions.MarkFalse(input.controlPlane.KCP, input.condition, input.unhealthyReason, kubekeyv1.ConditionSeverityWarning, "Following machines are reporting %s warnings: %s", input.note, strings.Join(kcpMachinesWithWarnings.List(), ", "))
		return
	}

	// In case of no errors, no warning, and at least one machine with info, report false, info.
	if len(kcpMachinesWithWarnings) > 0 {
		conditions.MarkFalse(input.controlPlane.KCP, input.condition, input.unhealthyReason, kubekeyv1.ConditionSeverityWarning, "Following machines are reporting %s info: %s", input.note, strings.Join(kcpMachinesWithInfo.List(), ", "))
		return
	}
