...
// infrastructure template, and KubeadmConfig field need to be equivalent.
func MatchesMachineSpec(machineSets map[string]*kubekeyv1.MachineSet, machineConfigs map[string]*bootstrapv1.KubeadmConfig, kcp *controlplanev1.ControlPlane) func(machine *kubekeyv1.Machine) bool {
	return collections.And(
		func(machine *kubekeyv1.Machine) bool {
			return matchMachineTemplateMetadata(kcp, machine)
		},
		MatchesKubeadmBootstrapConfig(machineConfigs, kcp),
		MatchesTemplateClonedFrom(machineSets, kcp),
	)
}

// MatchesTemplateClonedFrom returns a filter to find all machines that match a given KCP infra template.
func MatchesTemplateClonedFrom(machineSets map[string]*kubekeyv1.MachineSet, kcp *controlplanev1.ControlPlane) collections.Func {
	return func(machine *kubekeyv1.Machine) bool {
		if machine == nil {
			return false
		}
		// MachineSet
		ms, found := machineSets[machine.Name]
		if !found {
			// Return true here because failing to get infrastructure machine should not be considered as unmatching.
			return true
		}

		clonedFromName, ok1 := ms.GetLabels()[kubekeyv1.MachineSetLabelName]
		if !ok1 {
		...
		if clonedFromName != kcp.Spec.Machines.InfrastructureRef.Name ||
			"MachineSet" != kcp.Spec.Machines.InfrastructureRef.GroupVersionKind().GroupKind().String() {
			return false
		}

		// Check if the machine template metadata matches with the infrastructure object.
		if !matchMachineTemplateMetadata(kcp, ms) {
			return false
		}
		return true
	}
...
func MatchesKubeadmBootstrapConfig(machineConfigs map[string]*bootstrapv1.KubeadmConfig, kcp *controlplanev1.ControlPlane) collections.Func {
	return func(machine *kubekeyv1.Machine) bool {
		if machine == nil {
			return false
		}
...
func matchClusterConfiguration(kcp *controlplanev1.ControlPlane, machine *kubekeyv1.Machine) bool {
...
func matchInitOrJoinConfiguration(machineConfig *bootstrapv1.KubeadmConfig, kcp *controlplanev1.ControlPlane) bool {
...
func getAdjustedKcpConfig(kcp *controlplanev1.ControlPlane, machineConfig *bootstrapv1.KubeadmConfig) *bootstrapv1.KubeadmConfigSpec {
...
func matchMachineTemplateMetadata(kcp *controlplanev1.ControlPlane, obj client.Object) bool {
	// Check if annotations and labels match.
	if !isSubsetMapOf(kcp.Spec.Machines.ObjectMeta.Annotations, obj.GetAnnotations()) {
		return false
	}
	if !isSubsetMapOf(kcp.Spec.Machines.ObjectMeta.Labels, obj.GetLabels()) {
		return false
	}
	return true
}
