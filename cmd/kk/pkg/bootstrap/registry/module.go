...
import (
	...
	dockertemplate "github.com/kubesphere/kubekey/cmd/kk/pkg/container/templates"
...
func InstallHarbor(i *InstallRegistryModule) []task.Interface {
	...
	generateDockerService := &task.RemoteTask{
		...
		Action: &action.Template{
			Template: dockertemplate.DockerService,
			Dst:      filepath.Join("/etc/systemd/system", dockertemplate.DockerService.Name()),
		},
		Parallel: true,
	}
	...
	generateDockerConfig := &task.RemoteTask{
		...
		Action: &action.Template{
			Template: dockertemplate.DockerConfig,
			Dst:      filepath.Join("/etc/docker/", dockertemplate.DockerConfig.Name()),
			Data: util.Data{
				"Mirrors":            dockertemplate.Mirrors(i.KubeConf),
				"InsecureRegistries": dockertemplate.InsecureRegistries(i.KubeConf),
			},
		},
		Parallel: true,
	}
