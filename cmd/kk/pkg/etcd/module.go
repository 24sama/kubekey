...
import (
	kubekeyapiv1alpha2 "github.com/kubesphere/kubekey/cmd/kk/api/v1alpha2"
...
func (c *CertsModule) IsSkip() bool {
	return c.Skip
}
...
func (i *InstallETCDBinaryModule) IsSkip() bool {
	return i.Skip
}
...
func (c *ConfigureModule) IsSkip() bool {
	return c.Skip
}

func (c *ConfigureModule) Init() {
	c.Name = "ETCDConfigureModule"
	c.Desc = "Configure ETCD cluster"

	if v, ok := c.PipelineCache.Get(common.ETCDCluster); ok {
		cluster := v.(*EtcdCluster)
		if !cluster.clusterExist {
			c.Tasks = handleNewCluster(c)
		} else {
			c.Tasks = handleExistCluster(c)
		}
	}
}
...
func (b *BackupModule) IsSkip() bool {
	return b.Skip
}
